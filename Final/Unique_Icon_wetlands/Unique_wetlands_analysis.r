...
#data.dir="C:/Users/jc246980/Documents/Current projects/MD Vegetation/"; setwd(data.dir)
...
write.csv(wetlandsALLV2PA , file = "Full_TLM_dataset_PA.csv") # save data out for report Appendix 

my_species=colnames(wetlandsALLV2PA)
my_speciesV2 =mycodes[which(mycodes$sp_code_clean %in% my_species),] 
my_speciesV3=my_speciesV2[,!duplicated(my_speciesV2[c('sp_code_clean')])]
write.csv(my_speciesV3 , file = "TLM_species.csv") # save data out for report Appendix 

# Check against victorian flora
...
tada=tada[ , -which(names(tada) %in% c("Blu.alga"))]
tadaPA<-decostand(tada,method="pa")

dis <- vegdist(tadaPA)
groups <- factor(c(rep(1,10), rep(2,12), rep(3,15), rep(4,10), rep(5,14), rep(6,11)), labels = c("Hattah","LMW","Chowilla","Gunbower","KP","Barmah"))
...
png(paste(image.dir,"Wetlands together NMDS May 2019.png",sep=''),width=12.5, height=12, units='cm', res=500, pointsize=10, bg='white')
...
tada = matrix(NA,nrow=7,ncol=4)#define the output matrix
rownames(tada)=c("ALL","HAT", "LMW", "CHOW", "GUN", "KP", "BARM")
colnames(tada)=c("n","BTotal", "BRepl", "BRich")
...
mine<-beta.multi(tdata,index.family="sorensen")
tada[1,1] = nrow(tdata)
tada[1,2] = mine[[1]]
tada[1,3] = mine[[2]]
tada[1,4] = mine[[3]]
...
write.csv(tada , file = "Wetlands beta analysis PAdata May 2019.csv") # save data out
#########################################################################################################
...
png(paste(image.dir,"Wetlands Dry Exotics NMDS May 2019.png",sep=''),width=12.5, height=12, units='cm', res=500, pointsize=10, bg='white')
        par(mar=c(4,4,1,1),mfrow=c(2,1),cex=1,oma=c(2,0,1,0.5)) #defines plot parameters mfrow is number of images per sheet in this case its 5 rows by two columns
