...
xLabels <- c('FDFD','FDDD','DDFD','DDDD','FFFF')

tiff('Fig_TreatmentMeansP2_LA.tiff', width = 16, height = 16, units = 'cm', res = 300)
...
for (Y in yStrings) {
  ...
  X <- as.numeric(SLstats$Treat) # Numeric to stop automatic boxplot when parsed factor
  ...
  plot(X, y = Ymean, axes = FALSE, xlab = '', ylab = '', type = 'n', ylim = c(0,yMax), xlim = c(0.5,5.5))
  # Lines:
  #for (s in spp) {
  ...
  axis(1, labels = xLabels, at = 1:5, las = 3)
  axis(2)
  l = letters[pCount]
  ...
  if (pCount == 8) {mtext('Watering treatment', side = 1, line = 4, at = 3, cex = 0.8)}
...
yLabels <- c('Above:Below length (cm)')
			 
xLabels <- c('FDFD','FDDD','DDFD','DDDD','FFFF')

tiff('Fig_TreatmentMeansP2_LengthAB.tiff', width = 16, height = 16, units = 'cm', res = 300)

myPar <- par(mfrow = c(1,1), mar = c(1,1,1,1), oma = c(1,1,1,1))
pCount <- 1


  Ymean <- SLstats$LengthABMean
  YSE <- SLstats$LengthABSD
  X <- as.numeric(SLstats$Treat) 
  ...
  arrows(X, Ymean, X, Ymean-1.96*YSE, length = 0, angle = 90)
  points(X, Ymean, pch = myPch, cex = pointCex, bg = myBg)
  axis(1, labels = xLabels, at = 1:5, las = 1)
  axis(2)
  l = letters[pCount]
  title(main = paste('(',l,')', sep = ''), font.main = 1, adj = 0)
  title(ylab = yLabels[pCount])
  ...
  if (pCount == 1) {mtext('Watering treatment', side = 1, line = 4, at =1, cex = 0.8)}
