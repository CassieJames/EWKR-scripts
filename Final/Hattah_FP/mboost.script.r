...
set.seed(806)
indvecL <-sample(1:n,n,replace=FALSE) # create a random set of numbers
datenSmallrich <- daten.rich[indvecL,][1:(n/2),] # create a subset of half the original data using the random numbers
...
set.seed(1)
n<-nrow(datenSmallrich)
indvec<-sample(1:n,n,replace=TRUE)
traindatarich<-datenSmallrich[indvec,]
traindatav2rich=traindatarich[,-1] # remove site names from data frame

# Run model

Full.model.rich <-gamboost(formulaB,data=traindatav2rich, family=Gaussian(), control=boost_control(mstop=1000,trace=TRUE)) # original paper has mstop= 10000 but I reduced down the mstop value for trial runs as it takes a while
# This churns out a couple of warnings that I don't fully understand regarding the linear effects - "covariates should be (mean-) centered if intercept =False for Easting, Northing and d30"
mopt <- mstop(aic <- AIC(Full.model.rich)) 

# Carry out 5 fold cross validation to determine optimal stopping iteration  - increase cross validation for proper model runs?
cv5f <- cv(model.weights(Full.model.rich), type='kfold', B=5)
cvm <- cvrisk(Full.model.rich, folds=cv5f)
plot(cvm)
st<-(mstop(cvm))
Full.model.rich[st]

# Example of marginal functional estimates of boosted additive models for flood frequency, time since last flood and rainfall in  days prior to sampling
# rem that data has been centred so when looking at the plots its helpful to 'uncentre' the data
mFf<-mean(envdata_backup$Flood_frequency)
...
# the below line had to adapted from the paper to correct for the type of quotation marks, the order of the components and the spacing around the equals signs: see coef(Full.model) 
yvalues=xmatSmooth[[1]]%*%coef(Full.model)$`bbs(Flood_frequency, df = 1, center = TRUE)` + xmatLin[[1]]*coef(Full.model)$`bols(Flood_frequency, intercept = FALSE)`
plot(sort(traindata$Flood_frequency+mFf),yvalues[order(traindata$Flood_frequency+mFf)], type="l",xlab='Flood frequency', ylab='f(Flood frequency)')
rug(sort(traindata$Flood_frequency+mFf))
