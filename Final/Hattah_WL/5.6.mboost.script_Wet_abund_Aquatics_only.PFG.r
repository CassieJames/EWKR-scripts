...
Model_covary_interact_Wet_Natives <-gamboost(form_covary_interact,family = Binomial(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.01)) 
...
png("Aqua_Natives NegLL comparing different models df1.png",width=25, height=25, units='cm', res=300, pointsize=20, bg='white')
...
sel_aqua_Natives <- stabsel(Model_covary_flow_Aqua_Natives,cutoff=0.75, q = 5)
...
# Pseudo r2 but see below for resampled estimates
		...
		m1 <- glm.nb(Aqua_Natives ~ 1, data=daten) # null model	
		y <- daten$Aqua_Natives
		n <- length(y)
		r2 <- ( 1 - exp( - 2/n * (logLik(Model_covary_flow_Aqua_Natives_final) - logLik(m1)) ) ) / ( 1 - exp(logLik(m1))^{2/n})
