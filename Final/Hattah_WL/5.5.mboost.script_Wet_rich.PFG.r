...
tmp.abund.wet <- data.frame(cv = c(extrb(cv_spatial_WetNatRich), extrb(cv_covar_WetNatRich), extrb(cv_covar_climate_WetNatRich),extrb(cv_covar_flow_WetNatRich),extrb(cv_covarspatial_WetNatRich),
				 extrb(cv_tree_WetNatRich),extrb(cv_tree_spatial_WetNatRich)),
				model = gl(7, length(extrb(cv_spatial_WetNatRich))),b = factor(rep(1:length(extrb(cv_spatial_WetNatRich)), 7)))
...
png("WetNatRich NegLL comparing different models May 2019.png",width=25, height=25, units='cm', res=300, pointsize=20, bg='white')
...
boxplot(cv ~  model, data = tmp.abund.rich, axes = FALSE,
		...
		axis(2)
		out <- tapply(1:nrow(tmp.abund.rich), tmp.abund.rich$b, function(x) lines(1:7, tmp.abund.rich[x,"cv"], col = rgb(0,0,0,0.1)))
...
load("c:/Users/jc246980/Documents/Current projects/MD Vegetation/Rdata files/predicted.in.rich.RData")
