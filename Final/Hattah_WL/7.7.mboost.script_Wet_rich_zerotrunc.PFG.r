...
fitD <- dnbinom(0:20, size=0.9233317, mu=2.4783983)
...
envdata[which(envdata$WaterYr==y & envdata$Site.ID.x==s),c("WetNatRich_T1")]<-envdata[which(envdata$WaterYr==y-1& envdata$Site.ID.x==s),c("WetNatRich")]
}

# Do 2016 separately as will have to use 2014 result as predictor as no 2015 data
envdata[which(envdata$WaterYr==2016 & envdata$Site.ID.x==s),c("WetNatRich_T1")]<-envdata[which(envdata$WaterYr==2014 & envdata$Site.ID.x==s),c("WetNatRich")]
}
...
envdata$WetNatRich_T1=as.numeric(scale((envdata$WetNatRich_T1),center=TRUE, scale=FALSE))
...
Model_legacy_WetNatRich <-mboost(form_legacy,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # orig
Model_spatial_WetNatRich <-mboost(form_spatial,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_WetNatRich <-gamboost(form_covary,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_climate_WetNatRich<-gamboost(form_covary_climate,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # orig
Model_covary_flow_WetNatRich<-gamboost(form_covary_flow,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # orig
Model_covary_spatial_WetNatRich <-gamboost(form_covary_spatial,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_tree_spatial_WetNatRich <-mboost(form_tree_spatial,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE,nu=0.005)) # 
Model_tree_WetNatRich <-mboost(form_tree,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE,nu=0.005)) # 
Model_covary_interact_WetNatRich <-gamboost(form_covary_interact,family = Hurdle(),data = daten, control=boost_control(mstop=10000,trace=TRUE, nu=0.005)) 
...
tmp.abund.rich <- data.frame(cv = c(extrb(cv_legacy_WetNatRich),extrb(cv_spatial_WetNatRich), extrb(cv_covar_WetNatRich), extrb(cv_covar_climate_WetNatRich),extrb(cv_covar_flow_WetNatRich),extrb(cv_covarspatial_WetNatRich),
...
levels(tmp.abund.rich$model) <- nm



save(tmp.abund.rich, file = "c:/Users/jc246980/Documents/Current projects/MD Vegetation/Rdata files/tmp.abund.rich.RData") 
...
gbm.perspec(Wet_Rich_dismo, 6,2)
gbm.perspec(Wet_Rich_dismo, 15,2)
gbm.perspec(Wet_Rich_dismo, 13,1)
gbm.perspec(Wet_Rich_dismo, 5,2)
gbm.perspec(Wet_Rich_dismo, 6,5)
...
newdata=sorteddata[!sorteddata$WetNatRich==0,] # remove sites where there is no abundance

mytry <- gam(WetNatRich~s(d3Mon_meandepth)+s(log10(TSLW+1))+s(d3Mon_wet), family = nb(), data=newdata)
