...
# Run models
Model_legacy_Terr_Natives <-mboost(form_legacy,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # orig
Model_spatial_Terr_Natives <-mboost(form_spatial,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_Terr_Natives <-gamboost(form_covary,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_climate_Terr_Natives<-gamboost(form_covary_climate,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # orig
Model_covary_flow_Terr_Natives<-gamboost(form_covary_flow,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # orig
Model_covary_spatial_Terr_Natives <-gamboost(form_covary_spatial,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_tree_spatial_Terr_Natives <-mboost(form_tree_spatial,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE,nu=0.01)) # 
Model_tree_Terr_Natives <-mboost(form_tree,family = NBinomial(),data = daten, control=boost_control(mstop=1000,trace=TRUE,nu=0.01)) # 
...
tmp.abund.terr <- data.frame(cv = c(extrb(cv_legacy_Terr_Natives),extrb(cv_spatial_Terr_Natives), extrb(cv_covar_Terr_Natives), extrb(cv_covar_climate_Terr_Natives),extrb(cv_covar_flow_Terr_Natives),extrb(cv_covarspatial_Terr_Natives),
...
png("Terr_Natives NegLL comparing different models df1.png",width=25, height=25, units='cm', res=300, pointsize=20, bg='white')
...
boxplot(cv ~  model, data = tmp.abund.terr, axes = FALSE,
		...
		axis(2)
		out <- tapply(1:nrow(tmp.abund.terr), tmp.abund.terr$b, function(x) lines(1:8, tmp.abund.terr[x,"cv"], col = rgb(0,0,0,0.1)))
...
# Pseudo r2 but see below for resampled estimates
		...
		n <- length(y)
		r2 <- ( 1 - exp( - 2/n * (logLik(Model_tree_Terr_Natives) - logLik(m1)) ) ) / ( 1 - exp(logLik(m1))^{2/n})
...
#### Run model
		Model_tree_Terr_Natives.test <-mboost(form_tree,family = NBinomial(),data = datenSmall, control=boost_control(mstop=1000,trace=TRUE,nu=0.01)) # 
		cv5f <- cv(model.weights(Model_tree_Terr_Natives.test), type='subsampling', B=5)
		cv_tree_Terr_Natives.test<- cvrisk(Model_tree_Terr_Natives.test, folds=cv5f)
		st<-(mstop(cv_tree_Terr_Natives.test))
		Model_tree_Terr_Natives.test[st]
		mycoefs= coef(Model_tree_Terr_Natives.test) # store each model run coefficients incase we want to plot partial dependency plots for each test run to estimate ci
...
#### goodness of fit in-bootstrap
		...
		n.y <- length(y)
		r2 <- ( 1 - exp( - 2/n.y * (logLik(Model_tree_Terr_Natives.test) - logLik(m1)) ) ) / ( 1 - exp(logLik(m1))^{2/n.y})
		predicted.in[[i]] = r2
}
...
predictions<-predict(Model_tree_Terr_Natives,type='response')
