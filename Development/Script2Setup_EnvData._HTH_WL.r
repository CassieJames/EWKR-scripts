...
Hydrodata=Hydrodata[,c(3,5,19,20)] # tidy up and remove extra columns not needed

data.dir = "C:/Users/jc246980/Documents/Current projects/MD Vegetation/Environmental data/Hattah Lakes hydrology/"; setwd (data.dir) 
Bigmod=data.frame(read.csv("Hydraulics_HTH_WL.csv")) # import metrics determined from Bigmod model
Bigmod=Bigmod[,4:19]
Hydrodata=cbind(Hydrodata, Bigmod)
Hydrodata$TSLW[is.na(Hydrodata$TSLW)] <- 365*3
...
mydata_env=merge(mydata_env, Temperature.dat, by.x="Date.of.collection", by.y="Date",all.x=TRUE) # merge 
mydata_env=merge(mydata_env, VegStructure.dat, by="Site.ID", all.x=TRUE) # merge 



date.dir="C:/Users/jc246980/Documents/Current projects/MD Vegetation/Environmental data/"; setwd (date.dir) 
write.csv(mydata_env, file = "Hattah Lakes wetlands transect based env data.csv") # save data out to chec
