...
Depth.dat=read.csv("Hattah_sites_time_series_dataV2.csv", check.names=FALSE) # load hydrological information but prevent r form converting column names to a more 'friendly' format
...
Output= matrix(NA,nrow=nrow(Sites_sampled), ncol=18)
colnames(Output)=c("Site.ID", "Date.of.collection", "TSLW", "MRI_length", "MRI_shallow", "MRI_medium", "MRI_deep","d1yrs_dry", "d1yrs_wet","d1yrs_shallow", "d1yrs_medium", "d1yrs_deep", "d3yrs_dry", "d3yrs_shallow", "d3yrs_medium", "d3yrs_deep", "Freq_d1", "Freq_d3")
...
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),9] <- d1yrs_wet/365
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),10] <- d1yrs_shallow/365
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),11] <- d1yrs_medium/365
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),12] <- d1yrs_deep/365

Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),13] <- d3yrs_dry/(365+365+365)
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),14] <- d3yrs_shallow/(365+365+365)
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),15] <- d3yrs_medium/(365+365+365)
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),16] <- d3yrs_deep/(365+365+365)

Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),17] <- Freq_d1
Output[grepl(soi, Output$Site.ID, fixed=TRUE)& grepl(doi, Output$Date.of.collection),18] <- Freq_d3
