...
envdata=subset(envdata, envdata$TerrNatRich>0) # just model positive counts
...
form_spatial <- TerrNatRich~ bspatial(Easting, Northing, knots = 6, boundary.knots=NULL,df=df,center=TRUE,differences=1)+
...
form_covary_climate <- 	TerrNatRich~
				bols(d90, intercept=FALSE)+bbs(d90, center=TRUE, df=df)+
				bols(d365, intercept=FALSE)+bbs(d365, center=TRUE, df=df)+
				bols(MeanTemp90, intercept=FALSE)+bbs(MeanTemp90, center=TRUE, df=df)+
				bols(MinTemp90, intercept=FALSE)+bbs(MinTemp90, center=TRUE, df=df)+
...
form_covary_spatial <- TerrNatRich~ bspatial(Easting, Northing, knots = 6, boundary.knots=NULL,df=df,center=TRUE,differences=1)+
...
form_tree_spatial <- TerrNatRich~bspatial(Easting, Northing, knots = 6, boundary.knots=NULL,df=df,center=TRUE,differences=1)+
				bspatial(Easting, Northing, knots = 6, by=WaterYr,boundary.knots=NULL,df=df,center=TRUE,differences=1)+
...
# Run models
Model_spatial_TerrNatRich <-mboost(form_spatial,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_TerrNatRich <-gamboost(form_covary,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_covary_climate_TerrNatRich<-gamboost(form_covary_climate,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # orig
Model_covary_flow_TerrNatRich<-gamboost(form_covary_flow,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # orig
Model_covary_spatial_TerrNatRich <-gamboost(form_covary_spatial,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE, nu=0.01)) # originally 10000 but reduced down the mstop value for trial runs as it takes a while
Model_tree_spatial_TerrNatRich <-mboost(form_tree_spatial,family = MBztpoisson(),data = daten, control=boost_control(mstop=1000,trace=TRUE,nu=0.01)) # 

###

cv5f <- cv(model.weights(Model_spatial_TerrNatRich), type='subsampling', B=25)
cv_spatial_TerrNatRich <- cvrisk(Model_spatial_TerrNatRich, folds=cv5f)

cv5f <- cv(model.weights(Model_covary_TerrNatRich), type='subsampling', B=25)
cv_covar_TerrNatRich <- cvrisk(Model_covary_TerrNatRich, folds=cv5f)

cv5f <- cv(model.weights(Model_covary_climate_TerrNatRich), type='subsampling', B=25)
cv_covar_climate_TerrNatRich <- cvrisk(Model_covary_climate_TerrNatRich, folds=cv5f)

cv5f <- cv(model.weights(Model_covary_flow_TerrNatRich), type='subsampling', B=25)
cv_covar_flow_TerrNatRich<- cvrisk(Model_covary_flow_TerrNatRich, folds=cv5f)

cv5f <- cv(model.weights(Model_covary_spatial_TerrNatRich), type='subsampling', B=25)
cv_covarspatial_TerrNatRich<- cvrisk(Model_covary_spatial_TerrNatRich, folds=cv5f)

cv5f <- cv(model.weights(Model_tree_spatial_TerrNatRich), type='subsampling', B=25)
cv_tree_spatial_TerrNatRich<- cvrisk(Model_tree_spatial_TerrNatRich, folds=cv5f)


st<-(mstop(cv_spatial_TerrNatRich))
Model_spatial_TerrNatRich[st]

st<-(mstop(cv_covar_TerrNatRich))
Model_covary_TerrNatRich[st]

st<-(mstop(cv_covar_climate_TerrNatRich))
Model_covary_climate_TerrNatRich[st]

st<-(mstop(cv_covar_flow_TerrNatRich))
Model_covary_flow_TerrNatRich[st]

st<-(mstop(cv_covarspatial_TerrNatRich))
Model_covary_spatial_TerrNatRich[st]

st<-(mstop(cv_tree_spatial_TerrNatRich ))
Model_tree_spatial_TerrNatRich[st]
...
tmp.rich.ter <- data.frame(cv = c(extrb(cv_spatial_TerrNatRich), extrb(cv_covar_TerrNatRich), extrb(cv_covar_climate_TerrNatRich),extrb(cv_covar_flow_TerrNatRich),extrb(cv_covarspatial_TerrNatRich),
				 extrb(cv_tree_spatial_TerrNatRich)),
				model = gl(6, length(extrb(cv_spatial_TerrNatRich))),b = factor(rep(1:length(extrb(cv_spatial_TerrNatRich)), 6)))
				  

levels(tmp.rich.ter$model) <- nm
		
		
png("Terr_Richness NegLL comparing different models.png",width=25, height=25, units='cm', res=300, pointsize=20, bg='white')
		...
		text(seq_along(levels(tmp.rich.ter$model)), par("usr")[3] -0.005,srt = 30, adj = 1, label = levels(tmp.rich.ter$model), xpd = TRUE, font = 1, cex=0.7)
		axis(2)
...
sgmod_TerrNatRich <- summary(glht(lmer(cv ~ model + (1 | b), data = tmp.rich.ter), mcp(model = "Tukey")))
### Climate only model was the best predictor

### stability selection
sel_TerrNatRich <- stabsel(Model_covary_climate_TerrNatRich,cutoff=0.75, q = 2)
