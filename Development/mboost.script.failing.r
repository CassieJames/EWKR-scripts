...
#envdata$Date.of.collection <-as.Date(as.character(envdata$Date.of.collection), format = "%d/%m/%Y")
...
par(mfrow=c(1,3))
plot(residuals(Full.model), ylab="Residuals",xlab="Year")
acf(residuals(Full.model), main="ACF of residuals")
pacf(residuals(Full.model), main="ACF of residuals")